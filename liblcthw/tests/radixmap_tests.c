...
static char *test_operations(){
  ...
  timer_start(&start);
  mu_assert(make_random(map), "Didn't make random fake radix map.");
  timer_end(&start,&end,"randomizing the map");

  timer_start(&start);
  RadixMap_sort(map);
  timer_end(&start,&end,"sorting the map");
  
  timer_start(&start);
  mu_assert(check_order(map),"Failed to properly sort radix map");
  timer_end(&start,&end,"checking the order");

  mu_assert(test_find(map),"Failed the search test");
  mu_assert(check_order(map),"Map didn't stay sorted after search");
  ...
  while(map->end > 0){
    ...
    mu_assert(RadixMap_delete(map,el) == 0,"Could not delete element");
    mu_assert(old_end - 1 == map->end,"Wrong size after deletion");

    mu_assert(check_order(map),"RadixMap is not sorted after delete");
  ...
  */
  ...
  timer_end(&start,&end,"destroying the map");
